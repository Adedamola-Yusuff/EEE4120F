...
function run_analysis()
    ...
    for i = 1:length(names)
        fprintf("Measuring serial execution time for image size %s", names(i))
    ...
    for w = 1:length(workers)
        ...
        parpool(workers(w))
        for i = 1:length(names)
            fprintf("Measuring parallel execution time for image size %s with %d workers", names(i), workers(w))
            width = image_sizes(i,1);
            height = image_sizes(i,2);
    ...
    for i = 1:length(results)
        for w = 1:length(results(i).parallel)
            fprintf("Measuring speedup on image size %s when comparing parallel with %d workers and serial", names(i), workers(w))
    ...
    for i = 1:length(results)
        % Save serial image
        fprintf("Saving mandelbrot image from serial computation for %s", name(i))
        ...
        for w = 1:length(results(i).parallel)
            fprintf("Saving mandelbrot image from parallel computation with %d workers for image size %s", workers(w), names(i))
    ...
    fprintf("Making runtime comparison plot")
    ...
    for w = 1:length(workers)
        ...
        plot(parallel_times,'LineWidth',2)
    
    end
    
    legend(["Serial","2 workers","3 workers","4 workers","5 workers","6 workers"])
    xlabel("Resolution Index")
    ylabel("Execution Time (s)")
    title("Serial vs Parallel Mandelbrot Execution Time")
    grid on
    
    saveas(gcf,"runtime_comparison.png")

    %% ===== Speedup vs Workers Plot =====
    fprintf("Making speedup vs workers plot")
    ...
    figure
    plot(workers, speedups,'-o','LineWidth',2)
    
    xlabel("Number of Workers")
    ylabel("Speedup")
    title(sprintf("Speedup vs Workers (%s Resolution)", results(target).name))
    ...
    fprintf("Making speedup vs resolution plot")
    figure
    hold on
    ...
    legend(["2 workers","3 workers","4 workers","5 workers","6 workers"])
    xlabel("Resolution Index")
    ylabel("Speedup")
    title("Speedup vs Resolution")
    grid on
